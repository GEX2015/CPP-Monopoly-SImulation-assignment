...
public:
	...
	Place nearestRailroad() const;
	Place nearestUtility() const;
	const Place getPos();
	CommunityChest drawCommunityChest() const;
	Chance drawChance() const;
	void results();
	std::string placeToString(Place) const;
	void fillMaps();
	
private:

	Place _pos; //current position on the board
	std::map<Place, int> _placeTouches; //map of the enum and the amount of times it was landed on
	std::map<Place, std::string> _placeNames;//map of the enum and the string to print out in the results
};
